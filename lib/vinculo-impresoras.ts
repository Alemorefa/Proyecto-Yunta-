...
import { crearImpresora, editarImpresora, moverImpresoraDeTienda, cambiarEstadoImpresora, type Impresora } from "./impresoras-data";
...
export async function sincronizarImpresoraDesdeActivo(activo: Activo): Promise<void> {
  ...
  if (activo.printer_id) {
    ...
    const nueva = await crearImpresora(modelo, activo.store_id);
