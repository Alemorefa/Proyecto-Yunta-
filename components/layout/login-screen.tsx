...
import { iniciarSesion, recuperarContrasena } from "@/lib/auth";
...
export function LoginScreen() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const resultado =
      modo === "login" ? await iniciarSesion(email, contrasena) : await recuperarContrasena(email);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--navy-900)] p-4">
      ...
      >
        ...
        <p className="mt-4 text-center text-[11px] text-white/30">
          ¿Sos nuevo acá? Pedile a un administrador que te dé de alta desde la pantalla de Usuarios.
        </p>
      </form>
    </div>
  );
